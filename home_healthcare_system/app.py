...
def demo_switch():
    ...
    elif target_role in ("pharmacist", "pharm", "chetna"):
        cursor.execute("SELECT * FROM users WHERE email = 'pharm.chetna@demo.com' LIMIT 1")
...
def process_payment(app_id):
    ...
    return jsonify({
        "success": True,
        "message": f"Payment of ${total:.2f} completed successfully via {payment_method.replace('_', ' ').title()}!",
